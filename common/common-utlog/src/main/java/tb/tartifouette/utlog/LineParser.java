...
public class LineParser {
    ...
    private static final Pattern ASSIST_LINE =
            Pattern.compile("\\s*\\d+:\\d+\\s+Assist:\\s\\d+\\s\\d+\\s\\d+:\\s+(\\S+)\\sassisted (\\S+).*");

    private static final Pattern FLAG_LINE = Pattern.compile("\\s*\\d+:\\d+\\s+Flag:\\s(\\d+)\\s(\\d+):\\s(.*)");

    private static final Pattern STATS_LINE =
            Pattern.compile("\\s*(\\d+:\\d+)\\s+score:\\s(\\d+)\\s+ping:\\s+\\d+\\s+client:\\s+\\d+\\s+(.*)");

    private static final Pattern CONNECT_LINE = Pattern.compile(
            "\\s+\\d+:\\d+\\s+ClientUserinfo:\\s+(\\d+)\\s+\\\\ip\\\\(\\d+\\.\\d+\\.\\d+\\.\\d+):\\d+\\\\name\\\\(.*)\\\\racered.*");
    ...
    private void parseHit(String line) {
        ...
        if (matcher.matches()) {
            ...
            BodyZone zone = HitResolver.getInstance().resolveBodyZone(bodyPartId);
    ...
    private void parseAssist(String line) throws ParseException {
        Matcher matcher = ASSIST_LINE.matcher(line);
        if (matcher.matches()) {
            String assistant = matcher.group(1);
            String assiste = matcher.group(2);
            assistant = aliasManager.resolveUserName(assistant);
            assiste = aliasManager.resolveUserName(assiste);
            stats.updateAssistant(assistant);
            stats.updateAssiste(assiste);
        }
    }
