...
public class ReportGenerator {
    ...
    public void generateReport() throws IOException {
        log.info("Possible aliases : " + AliasManager.getInstance().getPossibleAliases());
    ...
    private void generateWeaponStats() throws IOException {
        ...
        try {
            ...
            for (Entry<WeaponPerKiller, Integer> entry : weapons.entrySet()) {
                ...
                writer.write(user.getKiller() + ";" + user.getWeapon() + ";" + entry.getValue()
                        + EOL);
            }
        } finally {
    ...
    private void generateUserDetailedStats() throws IOException {
        Writer writer = null;
        try {
            log.info("Writing report file " + destDirectory + "/users-details.csv");
            ...
            for (Entry<WhoKilledWhoWithWhat, Integer> entry : users.entrySet()) {
                ...
                writer.write(user.getKiller() + ";" + user.getKilled() + ";" + user.getWeapon()
                        + ";" + entry.getValue() + EOL);
            }
        } finally {
            IOUtils.closeQuietly(writer);
    ...
    private void generateScorePerMapStats() throws IOException {
        ...
        try {
            ...
            for (Entry<UserMap, UserScore> user : users.entrySet()) {
                ...
                writer.write(user.getKey().getUser() + ";" + user.getKey().getMap() + ";"
                        + value.getTotalScore() + ";" + value.getNbPlays() + ";"
                        + value.getTotalFrags() + ";" + value.getTotalDeaths() + ";"
                        + value.getTotalSuicides() + ";" + value.getTotalEnvironment() + ";"
                        + value.getFlags() + ";" + value.getTeamKiller() + ";"
                        + value.getTeamKilled() + ";" + value.computeScorePerPlay() + ";"
                        + value.computeFragPerPlay() + ";" + value.computeFlagPerPlay() + ";"
                        + value.computeFragPerDeathRatio() + ";" + value.getBestFragSerie() + ";"
                        + value.getBestFragSerieDurationInS() + ";" + (-value.getWorseKillSerie())
    ...
    private void generateScoreStats() throws IOException {
        ...
        try {
            ...
            for (Entry<String, UserScore> user : users.entrySet()) {
                UserScore value = user.getValue();
                writer.write(user.getKey() + ";" + value.getTotalScore() + ";" + value.getNbPlays()
                        + ";" + value.getTotalFrags() + ";" + value.getTotalDeaths() + ";"
                        + value.getTotalSuicides() + ";" + value.getTotalEnvironment() + ";"
                        + value.getFlags() + ";" + value.getTeamKiller() + ";"
                        + value.getTeamKilled() + ";" + value.computeScorePerPlay() + ";"
    ...
    private void generateMapStats() throws IOException {
        ...
        try {
            ...
            writer.write("Map;drapeaux bleus;drapeaux rouges;%victoires bleues" + EOL);
            ...
            for (Entry<String, MapResult> map : maps.entrySet()) {
                ...
                writer.write(map.getKey() + ";" + result.getFlagBlue() + ";" + result.getFlagRed()
                        + ";" + ratio + EOL);
            }
        } finally {
            IOUtils.closeQuietly(writer);
